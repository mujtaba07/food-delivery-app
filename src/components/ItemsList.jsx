...
const ItemsList = ({ item , addBtn }) => {
  console.log("addbtn",addBtn)
    ...
    return (
      <div>
        {item?.map((eachItem) => (
          <div key={eachItem.card.info.id} className="dish-item-wrapper my-10 flex gap-4 border-b-2 p-4 items-center justify-between">
            ...
            {addBtn && (
              <button
                className="bg-white border font-semibold text-[18px] text-[rgb(27,166,114)] leading-[22px] tracking-[-0.45px] text-md rounded-md py-1 px-6 absolute left-9 -bottom-3"
                onClick={() => handleAddItem(eachItem)}
              >
                Add+
              </button>
            )}
